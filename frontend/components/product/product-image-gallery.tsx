...
export function ProductImageGallery({
  ...
  const isVerticalThumbs = layout === 'vertical-thumbs'

  // Hauptbild — gemeinsames JSX fuer beide Layout-Varianten
  const mainImage = (
    <div
  ...
  // Mobile dots — bleiben in beiden Layouts unveraendert (lg:hidden)
  const mobileDots = images.length > 1 && (
    <div
  ...
  // Vertical-Thumbs-Layout (Desktop): 80px-Spalte links, Main rechts.
  // Mobile: Stack — Main + Dots; vertical-thumbs greift erst ab lg:.
  if (isVerticalThumbs) {
    return (
      <div className="lg:grid lg:grid-cols-[80px_1fr] lg:gap-4">
        {/* First grid column on desktop: vertical thumbs */}
        {images.length > 1 && (
          <div
        ...
        {/* Second grid column on desktop: main image (+ mobile dots) */}
        <div>
          {mainImage}
          {mobileDots}
        </div>
  ...
  // Default-Layout: Hauptbild + Dots (Mobile) / horizontale Thumbnail-Reihe (Desktop)
  return (
    <div>
      {mainImage}
      ...
      {images.length > 1 && (
        <>
          ...
          {/* Desktop: Thumbnail-Reihe (horizontal) */}
          <div
